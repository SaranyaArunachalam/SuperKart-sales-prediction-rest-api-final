...
def predict_sales():
    ...
    if isinstance(prediction, float) and (np.isinf(prediction) or np.isnan(prediction)):
        prediction = None
...
def predict_sales_batch():
    ...
    # Replace inf and nan with None for JSON serialization in batch predictions
    predicted_sales_clean = []
    for sale in predicted_sales:
        if isinstance(sale, float) and (np.isinf(sale) or np.isnan(sale)):
            predicted_sales_clean.append(None)
        else:
            predicted_sales_clean.append(sale)
    predicted_sales = predicted_sales_clean
